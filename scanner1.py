...
def merge_nonempty(dst: dict[str, Any], src: dict[str, Any] | None, preserve: set[str] | None = None) -> dict[str, Any]:
    """
...
def precheck(ip: str, city: dict[str, Any]) -> list[str]:
    ...
    # RouterOS API: plain TCP/8728 only. Never use API-SSL/8729.
    if (
        int(city.get("api_enabled", 1))
        and int(city.get("api_ssl_enabled", 0)) == 0
        and tcp_open(ip, int(city["api_port"]))
    ):
        reasons.append(f"API {city['api_port']}")

    # SSH fallback
    if tcp_open(ip, int(city["ssh_port"])):
        reasons.append("SSH")
...
def normalize_api_data(api_data: dict[str, Any] | None) -> dict[str, Any]:
    ...
    def field(obj: dict[str, Any], *names: str) -> str:
        for name in names:
            if name in obj:
                value = s(obj.get(name))
                if value:
                    return value
        return ""
    ...
    for src, dst in (
        ...
        value = s(api_data.get(src))
        if value:
            out[dst] = value

    if s(api_data.get("board_name")) and not s(out.get("model")):
        out["model"] = s(api_data["board_name"])

    total = numeric_value(api_data.get("memory_total"))
    free = numeric_value(api_data.get("memory_free"))
    if total is not None and free is not None and total > 0:
        out["memory_usage"] = f"{max(0.0, (1.0 - free / total) * 100.0):.1f}%"

    if s(api_data.get("temperature")):
        out["temperature"] = s(api_data["temperature"])

    if s(api_data.get("cpu_temperature")):
        out["cpu_temperature"] = s(api_data["cpu_temperature"])

    radio = api_data.get("radio") or {}
    if isinstance(radio, list):
        radio = radio[0] if radio else {}
    if not isinstance(radio, dict):
        radio = {}

    mappings = {
        "ssid": ("ssid", "ssid-name", "network-name"),
        "frequency": ("frequency", "freq"),
        "channel": ("channel", "channel-id", "channel-number"),
        "bandwidth": ("bandwidth", "channel-width", "channel-widths"),
        "mode": ("mode", "station-mode"),
        "wireless_status": ("status", "state", "wireless-status"),
        "tx_power": ("tx-power", "tx-power-current"),
        "rx_power": ("rx-power", "receive-power"),
        "signal_strength": ("signal-strength", "signal", "rx-signal"),
        "peer_tx_signal": ("tx-signal", "peer-tx-signal", "remote-signal"),
        "tx_rate": ("tx-rate", "tx-rate-set", "tx-rates"),
        "rx_rate": ("rx-rate", "rx-rate-set", "rx-rates"),
        "ccq": ("ccq", "overall-ccq"),
        "tx_ccq": ("tx-ccq",),
        "rx_ccq": ("rx-ccq",),
        "snr": ("snr", "signal-to-noise"),
        "noise_floor": ("noise-floor", "noise"),
        "radio_name": ("interface", "interface-name", "name"),
    }

    for dst, aliases in mappings.items():
        value = field(radio, *aliases)
        if value:
            out[dst] = value

    if not s(out.get("interface_name")) and s(api_data.get("interface_name")):
        out["interface_name"] = s(api_data["interface_name"])

    if not s(out.get("radio_name")) and s(out.get("interface_name")):
        out["radio_name"] = out["interface_name"]
